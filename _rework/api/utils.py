...
data_skills_path = os.path.join(settings.BASE_DIR, "db_manager/datasets/skills_count.csv")
...
def skill_cleaner(data):
    ...
    for element in data:
        ...
        element = element.replace('Data', 'Data Analysis')
        ...
        element = element.replace('Excel', 'Microsoft Excel')
        element = element.replace('matlab', 'Matlab')
        element = element.replace('MySQL', 'SQL')
        element = element.replace('Office', 'Microsoft Office')
