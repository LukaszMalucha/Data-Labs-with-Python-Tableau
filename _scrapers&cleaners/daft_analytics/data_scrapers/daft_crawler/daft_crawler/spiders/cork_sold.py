...
class CorkSoldSpider(Spider):
	name = 'cork_sold'
	allowed_domains = ['www.daft.ie']
	start_urls = ['http://daft.ie/']

	def parse(self, response):

			self.driver = webdriver.Chrome('D:/PYTHON/WebScraping/chromedriver')  ## path to chromedriver on disk
			for i in range(1,61):												  ## 365 days of sales	
				self.driver.get('https://www.daft.ie/price-register/cork-city/?min_price=25000&max_price=5000000&pagenum={0}'.format(i))
